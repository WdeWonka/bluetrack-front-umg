...
export function ImportModal({
  ...
  const downloadTemplate = async () => {
    ...
    try {
      await staffService.downloadTemplate();
      toast.dismiss();
  ...
  const handleConfirm = async () => {
    ...
    try {
      const data = await staffService.importFromExcel(file);
      ...
      // Extraer nombres de usuarios duplicados
      ...
      if (statusCode === 409 || (created_count === 0 && db_errors.length > 0)) {
        toast.error("Todos los usuarios ya existen en el sistema", {
          duration: 5000,
        });
      ...
      if (statusCode === 201 || (created_count > 0 && error_count === 0)) {
        ...
        toast.success(
          created_count === 1
            ? "Usuario importado correctamente"
            : "Todos los usuarios importados correctamente",
          { duration: 4000 },
        );
      ...
      if (statusCode === 207 || (created_count > 0 && error_count > 0)) {
        ...
        toast.success(
          `Se ${created_count === 1 ? "importó" : "importaron"} ${created_count} de ${totalProcessed} ${totalProcessed === 1 ? "usuario" : "usuarios"}`,
          { duration: 4000 },
        );
      ...
      // Fallback
      toast.success("Usuarios procesados", { duration: 4000 });
    } catch (error: any) {
