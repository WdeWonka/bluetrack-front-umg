...
export function ImportModal({
  ...
  const downloadTemplate = async () => {
    ...
    try {
      await warehouseService.downloadTemplate();
      toast.dismiss();
  ...
  const handleConfirm = async () => {
    ...
    const loadingToast = toast.loading("Importando almacenes...");

    try {
      const data = await warehouseService.importFromExcel(file);
      ...
      // Extraer nombres de almacenes duplicados
      ...
      if (statusCode === 409 || (created_count === 0 && db_errors.length > 0)) {
        toast.error("Todos los almacenes ya existen en el sistema", {
          duration: 1000,
        });
      ...
      if (statusCode === 201 || (created_count > 0 && error_count === 0)) {
        ...
        toast.success(
          created_count === 1
            ? "Almacén importado correctamente"
            : "Todos los almacenes importados correctamente",
          { duration: 4000 },
        );
      ...
      if (statusCode === 207 || (created_count > 0 && error_count > 0)) {
        ...
        toast.success(
          `Se ${created_count === 1 ? "importó" : "importaron"} ${created_count} de ${totalProcessed} ${totalProcessed === 1 ? "almacén" : "almacenes"}`,
          { duration: 4000 },
        );
      ...
      // Fallback
      toast.success("Almacenes procesados", { duration: 4000 });
    } catch (error: any) {
