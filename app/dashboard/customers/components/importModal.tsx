...
export function ImportModal({
  ...
  const handleConfirm = async () => {
    ...
    try {
      const data = await customerService.importFromExcel(file);
      ...
      // Extraer nombres de clientes duplicados
      ...
      if (statusCode === 409 || (created_count === 0 && db_errors.length > 0)) {
        toast.error("Todos los clientes ya existen en el sistema", {
          duration: 1000,
        });
      ...
      if (statusCode === 201 || (created_count > 0 && error_count === 0)) {
        ...
        toast.success(
          created_count === 1
            ...
            : "Todos los clientes importados correctamente",
          { duration: 4000 },
        );
      ...
      if (statusCode === 207 || (created_count > 0 && error_count > 0)) {
        ...
        toast.success(
          `Se ${created_count === 1 ? "importó" : "importaron"} ${created_count} de ${totalProcessed} ${totalProcessed === 1 ? "cliente" : "clientes"}`,
          { duration: 4000 },
        );
      ...
      // Fallback
      toast.success("Clientes procesados", { duration: 4000 });
    } catch (error: any) {
